...
SELECT o.order_id, od.quantity, date, DATETRUNC(hour,time) AS hour_ordered, p.pizza_id, pt.pizza_type_id, ROUND(p.price,2) AS price_two_decimals, pt.category
FROM orders AS o
...
*/
SELECT pizza_type_id, ROUND(SUM(price_two_decimals*quantity),2) AS pizza_type_rev
...
SELECT category, ROUND(SUM(price_two_decimals*quantity),2) AS category_rev, COUNT(DISTINCT pizza_type_id) AS num_in_category,
