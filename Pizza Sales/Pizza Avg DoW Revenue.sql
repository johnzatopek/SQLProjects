...
SELECT o.order_id, date, DATENAME(WEEKDAY, date) AS dow, DATETRUNC(hour,time) AS hour_ordered, p.pizza_id, ROUND(p.price,2) AS price_two_decimals
FROM orders AS o
...
revenue_per_day AS (
SELECT dow, SUM(price_two_decimals) AS revenue_per_day, COUNT(DISTINCT date) AS days_open
FROM orders_by_hour
GROUP BY dow
)
