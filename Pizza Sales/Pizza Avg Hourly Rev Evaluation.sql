...
SELECT o.order_id, od.quantity, date, DATETRUNC(hour,time) AS hour_ordered, p.pizza_id, ROUND(p.price,2) AS price_two_decimals
FROM orders AS o
...
SELECT date, hour_ordered, SUM(price_two_decimals*quantity) AS rev_on_day_in_hour, 
...
To save on operating costs, the store may want consider closing at 23:00 since only 23 order were made throughout the entire year in the 23:00 hour. */
...
SELECT date, hour_ordered, SUM(price_two_decimals*quantity) AS purchase_cost,
	SUM(SUM(price_two_decimals*quantity)) OVER (PARTITION BY hour_ordered) AS total_hour_revenue_for_year
FROM orders_by_hour
